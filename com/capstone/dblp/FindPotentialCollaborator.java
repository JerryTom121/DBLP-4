 ...
 * @author jhyeh
 */
public class FindPotentialCollaborator {
    private String name;
    ...
    public static void main(String[] args) {
        ...
        FindPotentialCollaborator fc = new FindPotentialCollaborator(args0, args[1], args[2], d);
        fc.doFind();
    }

    public FindPotentialCollaborator(String name, String afname, String dblpname, double d) {
        this.name = name;
        this.threshold = d;
        try {
    ...
    public void outputResult(Object[] array) {
        ...
        TreeSet coacoa = new TreeSet();
        for (int i=0; i<array.length; i++) {
            ASObject as = (ASObject)array[i];
            if (as.cosine < threshold)
                break;
            for (Iterator it=coa.iterator(); it.hasNext(); ) {
                String coaname = (String)it.next();
                if (as.label.equals(coaname))
                     continue;
                if (isCoAuthor(as.label, coaname) && (!as.label.equals(name))
                     && !isCoAuthor(as.label, name)) {
                     coacoa.add(as);
                }
            }
        }
        for (Iterator it=coacoa.iterator(); it.hasNext(); ) {
            ASObject as = (ASObject)it.next();
            range = getPublicationRange(as.label);
            System.out.println("Name = "+as.label+"("+range[0]+"-"+range[1]+
                        "), similarity = "+as.cosine);
        }
        System.out.println("Total "+coacoa.size()+" potential collaborators exist in two degree of separation."+
                "(threshold = "+threshold+")");
    }
