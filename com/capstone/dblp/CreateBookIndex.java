...
public class CreateBookIndex {
    ...
    public static void main(String[] args) {
        ...
        CreateBookIndex frc = new CreateBookIndex(args[0],
                                                           args[1], ys, ye);
        frc.doCreate();
    }

    public CreateBookIndex(String map, String titleidx, int ys, int ye) {
        this.year_start = ys;
        this.year_end = ye;
        this.mapname = map;
        this.bookmap = new HashMap();
        this.outfname = ""+ys+"-"+ye+".books.map";
        try {
    ...
    public void doCreate() {
        ...
        for (int i=0; i<keylist.length; i++) {
            ...
            String title = dblpobj.getBookTitle();
            double[] vec2 = (double[])bookmap.get(title);
            double[] newvec = addVec(vec, vec2);
            if (newvec != null) bookmap.put(title, newvec);
        }
        try {
